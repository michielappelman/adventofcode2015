...
r = re.compile('(.*) (\d*),(\d*) through (\d*),(\d*)')
instruct = []
for line in puzzle_input.split('\n'):
    m = r.findall(line)[0]
    instruct.append((m[0], int(m[1]), int(m[2]), int(m[3]), int(m[4])))
...
for ins in instruct:
    if "on" in ins[0]:
        turn_on2(ins[1], ins[2], ins[3], ins[4])
    elif "off" in ins[0]:
        turn_off2(ins[1], ins[2], ins[3], ins[4])
    elif "toggle" in ins[0]:
        toggle2(ins[1], ins[2], ins[3], ins[4])

len(lights.nonzero()[0])
lights.sum()